...
import { GithubUserList } from './commercial/dtos/GithubUserListDTO';
...
export class AppServices {
  ...
  getGithubUser = async (currentPage: number): Promise<GithubUserList[]> => {
    return new Promise((resolve, reject) => {
      serviceAdapter<GithubUserList[], undefined>(
        API_METHODS.GET,
        `${ServicesEndPoints.LIST_USER}?per_page=10&page=${currentPage}`
      )
        .then(res => {
          resolve(res);
        })
